        ...
        if ((inputText.length) > 0) {            
          $( '.mix').each(function() {
            $this = $("this");
             ...
             if($(this).children('.tab-block').text().toLowerCase().match(inputText)) {
          ...
          $("#responsivetabs").mixItUp('filter', $matching);
        }
  
        else {
          // resets the filter to show all item if input is empty
          $("#responsivetabs").mixItUp('filter', 'all');
        }
      }, 200 );
    });
  })
